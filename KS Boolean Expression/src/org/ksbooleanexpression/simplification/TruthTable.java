...
public class TruthTable {
	...
	private void addAndOperator() {
		...
		for (int i = 0; i < ch.length - 1; i++) {
			...
			if ((ch[i] <= 'Z' && ch[i] >= 'A' && ch[i + 1] <= 'Z' && ch[i + 1] >= 'A')
					|| (ch[i] <= 'Z' && ch[i] >= 'A' && ch[i + 1] == '(')
					|| (ch[i] == ')' && ch[i + 1] <= 'Z' && ch[i + 1] >= 'A')
					|| (ch[i] == ')' && ch[i + 1] == '(')
					|| (ch[i] >= 'A' && ch[i] <= 'Z' && ch[i+1] == '!'))
	 ...
	 *
	 * @param n
	 *            le nombre � convertir vers la binaire<br>
	 * @param lesBit
	 *            tableau qui contiendera les bit du nombre obtenu<br>
	 * @param nbrVar
	 *            le nombre de bit sur lesquel doit �tre �crit le r�sultat<br>
	 */
	 ...
	 * La table de vérité est donné sous forme d'un vecteur à 2 puissance le
	 * nombre<br>
	 * de variables.<br>
	...
	private void getTruthTable() {
		...
		if (variables.length() > 8)
			error = new SyntaxErrorException(
					Tools.getLocalizedString("TOO_MUCH_VARIABLES"));
