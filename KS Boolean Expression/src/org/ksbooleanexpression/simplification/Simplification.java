...
public class Simplification implements View {
	 ...
	 * Elle contient:<br>
	 * le nombre de term participant au regroupement l'adresse du regoupement un
	 * bool�en qui indique si le groupement est utilis� dans un autre
	 * regroupement pls important l'adresse des cellule participants � ce
	 * regroupement
	 *
	...
	private class KarnaughNode {
		...
		public void addCellAdress(String adress) {
			if (cellesAdress == null)
				cellesAdress = new ArrayList<String>();
	...
	public Simplification(boolean ttable[], int nbrVar, String ordrVar,
	 ...
	 *            l'ordre des variables
	 */
	public Simplification(boolean kmap[][], int nbrVar, String ordrVar,
			Controller controller) {
	 ...
	 *
	 * @param strings
	 *            les fonctions � simplifi�es
	 * @param solutionType
	 *            le type de solution
	 * @param controller
	 *            controller de l'application
	 */
	public Simplification(String[] strings, SolutionType solutionType,
			Controller controller) {
		s = "";
		if (solutionType.name().compareTo("DETAILLED_SOLUTION") == 0)
			this.detailledSolution = true;
		else
			this.detailledSolution = false;
		functions = strings;
		s += Tools.intialize();
	 ...
	 *
	 * @param n
	 *            le regoupement dont on veut v�rifier l'existence<br>
	 ...
	 *
	 * @param cellAdress
	 *            l'adresse de la celulle en d�cimal
	 * @param nbrVar
	...
	private String binaryCode(int cellAdress, int nbrVar) {
		...
		for (int i = 0; i < bt.length; i++)
			binaryAdress = binaryAdress + bt[i];
		return binaryAdress;

	}

	private void deleteUnneededNodes() {
		boolean[][] temp = new boolean[kmap.getRow()][kmap.getColumn()];
		boolean del = false;
		for (int i = 0; i < arrayList.size(); i++) {
	 ...
	 *
	 * @param ch
	 *            adresse du regroupement
	 * @param i
	 *            la position actuelle dans l'adresse du groupement
	...
	public String functionFromKmap(int nbrVar) {
		boolean truthTable[] = new boolean[kmap.getRow() * kmap.getColumn()];
	 ...
	 * @return l'expression de la fonction.
	 */
	private String getFunctionFromTruthTable(boolean truthT[], int nbrVar) {
		String function = "";
		int k = 65;
		for (int i = 0; i < truthT.length; i++) {
			...
			if (truthT[i]) {
				for (int j = 0; j < nbrVar; j++) {
					if ((i % ((int) Math.pow(2, nbrVar - j))) < ((int) Math
							.pow(2, nbrVar - j - 1)))
	 ...
	 *
	 * @param binAdress1
	 *            l'adresse de l'un des deux regroupement
	 * @param pos
	 *            la position du bit qui diff�re.
	 ...
	 */
	private int IsJoinable(KarnaughNode karnaughNode, KarnaughNode karnaughNode2) {
	 ...
	 *
	 * @param binAdress1
	 *            l'adresse de la celulle 1
	 * @param binAdress2
	 *            l'adresse de la celulle 2
	 ...
	 *
	 * @param type
	 *            forme de la fonction � simplifier.
	 */
	public void launchSimplification(Type type) {
		...
		case FUNCTION:
			for (String function : functions) {
				...
				if (truthTable.getError() != null) {
					this.s += Tools.writeError(i, truthTable.getError()
							.getMessage());
					i++;
					continue;
				}
				...
				if (detailledSolution && nombreVars != 1)
					this.s += Tools.writeKarnaughMap(kmap, nombreVars);
				if (functions.length == 1 && detailledSolution
						&& nombreVars != 1)
					this.s += Tools.writeTruthTable(truthTable, nombreVars,
							ordrVar);
				i++;
			}
			break;
		case TRUTH_TABLE:
			...
			function[0] = getFunctionFromTruthTable(truthTable.TruthTable,
					nombreVars);
			controller.program.getMainPanel().getTextField()
					.setText(function[0]);
	...
	private void minimisedExpression() {
		...
		for (int i = 0; i < arrayList.size(); i++) {
			...
			for (int j = 0; j < tabChar.length; j++) {
				if (tabChar[j] == '0') {
					ch = ch + "!" + ordrVar.charAt(j);
					if (!endOfTerms(arrayList.get(i).nodes, j))
						ch = ch + "*";
				} else {
					if (tabChar[j] == '1') {
						ch = ch + ordrVar.charAt(j);
		...
		for (int i = 1; i < x + 1; i++) {
			for (int j = 0; j < arrayList.size(); j++) {
				for (int k = 0; k < arrayList.size(); k++) {
					if (arrayList.get(j).numberOfItems == (int) Math.pow(2,
							i - 1)
							&& arrayList.get(k).numberOfItems == (int) Math
									.pow(2, i - 1)) {
						int y = IsJoinable(arrayList.get(j), arrayList.get(k));
						if (y != -1) {
							String s = getNewBinaryAdress(
									arrayList.get(j).nodes,
									IsJoinable(arrayList.get(j),
											arrayList.get(k)));
							KarnaughNode n = new KarnaughNode(s, false,
									arrayList.get(j).numberOfItems * 2);
							n.cellesAdress = getCellesAdress(arrayList.get(j),
									arrayList.get(k));
				...
				for (int i = 0; i < kmap.getRow(); i++) {
					for (int j = 0; j < kmap.getColumn(); j++) {
						if (kmap.getCellValue(i, j)) {
							...
							n.flag = false;
							n.numberOfItems = 1;
							n.nodes = binaryCode(kmap.getCellAdress(i, j),
									nombreVars);
