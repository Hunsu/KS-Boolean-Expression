...
public class PreferencePanel extends JDialog implements View {
	...
	public Preferences preferences = Preferences
			.userNodeForPackage(Program.class);
	...
	/**
	 * Cr�e et affiche une boite de dialogue pr�f�rences.
	 */
	public PreferencePanel(UserPreferences preferences,
		...
		setIconImage(img);
		setTitle(Tools.getLocalizedString(ActionType.PREFERENCES.name()
				+ ".Name"));
		...
		labelLangue = new JLabel(
				Tools.getLocalizedString(ActionType.PREFERENCES.name()
						+ ".Language")
						+ " :");
		...
		for (int i = 0; i < displayLanguages.length; i++) {
			...
			displayLanguages[i] = Character.toUpperCase(locale
					.getDisplayLanguage().charAt(0))
		...
		for (LookAndFeelInfo lf : laf) {
			if (lf.getClassName().compareTo(pref.getLookAndFeel()) == 0) {
		...
		lookAndFeelComboBox.setModel(new DefaultComboBoxModel<String>(lafs));
		...
		chckbxDetailedSol = new JCheckBox(
				Tools.getLocalizedString(ActionType.PREFERENCES.name()
						+ ".DetailedSolution"));
		...
		if (pref.getSolutionType().compareTo(SolutionType.DETAILLED_SOLUTION) == 0)
			chckbxDetailedSol.setSelected(true);
		else
			chckbxMiniFunc.setSelected(true);
		...
		cancelButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				SwingUtilities.updateComponentTreeUI(controller.program
						.getFrame());
				dispose();
			}
		});
	...
	private String getISOLanguageCode() {
		...
		for (int i = 0; i < languages.length; i++) {
			...
			if (locale.getDisplayLanguage().compareToIgnoreCase(
					languageComboBox.getSelectedItem().toString()) == 0)
				return languages[i];
		}
		return "en";
	}
	...
	protected void savePreferences() throws Exception {
		...
		pref.setLookAndFeel(pref.getLookAndFeelClassName(lookAndFeelComboBox
				.getSelectedItem().toString()));
		if (chckbxDetailedSol.isSelected())
			pref.setSolutionType(SolutionType.DETAILLED_SOLUTION);
		else
			pref.setSolutionType(SolutionType.MINIMIZED_FUNCTION);
	...
	protected void updateLookAndFeel() {
		try {
			UIManager.setLookAndFeel(pref
					.getLookAndFeelClassName(lookAndFeelComboBox
							.getSelectedItem().toString()));
		} catch (Exception e) {
		}
